...
contentExts=(`echo ${args[16]} | tr ';' ' '`)
ffmpegMachine=${args[17]} 
jobMachine=${args[18]} 
...
if [ -f $metaData3 ] ; then finalMetaFile=$metaData3
elif [ -f $metaData2 ] ; then finalMetaFile=$metaData2 
else echo "Error : Video has no meta information for rendering" ; exit ;
fi


for (( i=0; i<${#contentTypes[@]}; i++ ))
do
echo sed -i 's/content\ id=\"'$i'\"\ name=\"[0-9A-Za-z_.]*\"/content\ id=\"'$i'\"\ name=\"'${contentIDs[$i]}'\"/g' $finalMetaFile >> $jobScript
echo sed -i 's/content\ id=\"'$i'\"\ name=\"'${contentIDs[$i]}'\"\ ext=\"[0-9A-Za-z_.]*\"/content\ id=\"'$i'\"\ name=\"'${contentIDs[$i]}'\"\ ext=\"'${contentExts[$i]}'\"/g' $finalMetaFile >> $jobScript
done

echo ssh $jobMachine $exeDir"/renderContent" -blanks -md $metaDataDir $mainDir $contentDir $finalMetaFile  >> $jobScript
