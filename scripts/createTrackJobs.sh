...
$scriptDir"/"processVideo.sh $mainDir $videoDir $contentDir $metaDataDir"/"$jobID $exeDir $scriptDir $jobScriptDir $tempVideoDir $audioDir $jobID $videoName $outputVideoName $flashVideoName $contentTypes $contentIDs $contentNames ${ffmpegMachines[$ffMach]} ${jobMachines[$jobMach]}
