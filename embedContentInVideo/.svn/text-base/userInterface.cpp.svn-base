...
void MainWindow::on_options_menu(wxCommandEvent& event)
{
	...
	if( _canvas->_currentContent.points.size() == 4 )
		...
		for ( unsigned i=0 ; i<_canvas->contentNames.size() ; i++)
			message += _canvas->contentNames[i] + string(" ; ");
 		
	    message += string("\n\n") + string("Content ID ; Content  ; Render ; Track ; Occ ");
		
            wxTextEntryDialog dlgContent(this,message.c_str());

	
	    //Content name ; content type ; rendering type ; trackign type ; occ type 	

            if ( dlgContent.ShowModal() == wxID_OK )
            {
	        string tempInfo = (string)dlgContent.GetValue().c_str();

    	        vector<string> parts;
	        string del=";";
		tempInfo=tempInfo + del + "-1" + del;
		utilities::split(tempInfo,parts,del);

               _canvas->_currentContent.contentName = _canvas->contentNames[atoi(parts[0].c_str())];
               _canvas->_currentContent.contentType = atoi(parts[1].c_str());
               _canvas->_currentContent.renderType = atoi(parts[2].c_str());
               _canvas->_currentContent.trackType = atoi(parts[3].c_str());
               _canvas-> _currentContent.occType = atoi(parts[4].c_str());
               _canvas-> _currentContent.contentExtension = _canvas->contentExts[atoi(parts[0].c_str())];
...
void userInterface::OnInitCmdLine(wxCmdLineParser& parser)
{
   ...
   int fps=0;
    ...
    {
	string contentNames = string(str.c_str());
	cout<<contentNames.c_str()<<"\n";
	string del=";";
	utilities::split(contentNames,gMainWindow->_canvas->contentNames,del);
    }
    if (parser.Found("ext", &str)) 
    {
	string contentExts = string(str.c_str());
	string del=";";
	utilities::split(contentExts,gMainWindow->_canvas->contentExts,del);
    }
   ...
   if (parser.Found("label", &str)) 
	...
	vector<string> elems;
	string del=";";
	utilities::split(label,elems,del);
